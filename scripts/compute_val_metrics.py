...
from sparse_detector.utils.metrics import (
    gini_vectorized,
    zeros_ratio_vectorized,
    paibb_vectorized,
)
...
@click.option(
    "--metric-threshold", type=float, default=None, help="Threshold for metric"
)
...
def main(
    ...
    dist_config = dist_utils.init_distributed_mode(
        base_config["distributed"]["dist_url"]
    )
    ...
    if dist_config.distributed:
        model = torch.nn.parallel.DistributedDataParallel(
            model, device_ids=[dist_config.gpu]
        )

    matcher = None
    if metric == "paibb":
        print(
            "Building matcher for metric: Percentage of Attentions Inside Bounding Boxes"
        )
    ...
    for batch_id, (samples, targets) in enumerate(
        metric_logger.log_every(data_loader, log_freq=10, header=header, prefix="val")
    ):
    ...
    click.echo(
        f"Rank: {dist_config.rank}; Shape={rank_gini.shape}, Mean: {torch.mean(rank_gini, 0)}"
    )
